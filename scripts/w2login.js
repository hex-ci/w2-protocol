 ...
 *
 * 流程与客户端一致：
 *   1. SSO mlogin（账号密码，DES-ECB 加密）→ WST + 账号标识（字段名由 W2_SSO_USERID_FIELD 指定）
 *   2. 连选服服务器：
...
async function fetchServerList(username) {
  ...
  const r = await c.call(2, p.cat(
    ...
    p.str(config.loginParams()?.channel || ''),
...
(async function main() {
  ...
  const wst = sso.WST;
  const wtgt = sso.WTGT;
  // 键名来自 SSO 响应（服务器契约，勿改）；DES 解密后即 username
  // 账号标识字段名由 .env 的 W2_SSO_USERID_FIELD 指定（服务端下发，含厂商词根故不入库）
