...
import { DEFAULT_ARMY_ID, requireArmy, armyLabel } from '../lib/army.js';
import {
...
// 参数与 w2transport 对齐（默认值一致，口径才不会打架）：
//   底仓线小时数决定补底仓/富余判断的目标，油耗保留线决定油料预算。
// transport 特有的调度闸门（--max-trucks / --min-fill / --min-stack / --threshold）
// 不搬过来——手动挑量的场景没有「攒量阈值」「单笔车数上限」这些概念。
const FLOOR_HOURS = Math.max(1, parseInt(arg('hours', '24'), 10) || 24);
const FUEL_HOURS = Math.max(0, parseInt(arg('fuel-reserve', '6'), 10) || 6);
  ...
  if (phase === 'scan') {
    return html`
      <${Box} flexDirection="column">
        <${HeaderBar} connected=${!!rt.c?.connected} dry=${DRY} hubs=${hubLine} army=${armyLabel(ARMY_ID)} floorHours=${FLOOR_HOURS} fuelHours=${FUEL_HOURS} />
...
requireArmy(ARMY_ID); // 非法兵种 ID 立即报错并列出可用项
