...
import { RES_CN, trainQueueCap } from '../lib/formula.js';
import { createProgress } from '../lib/progress.js';
import { readTopologyCache } from '../lib/topology.js';
import {
  parseCityList, parse2003, parse2027, parse19009, parseBuildings49, parse3005, parse3006,
} from '../lib/proto.js';
...
(async function main() {
  ...
  for (const city of targetCities) {
    ...
    const busyPlants = activeQueues.length;

    // 队列口径：3005 每厂只回队首一条，真实深度只能逐厂查 3006（全域 120 厂全扫要多花一分钟）。
    // 显示只需「有空闲队列 / 已满」二元判断，故只查「全厂在产」的城：有空闲厂即有余量。
    // 先采样首厂——首厂未满即可判定有余量（1 次请求）；首厂已满才全扫确认整城是否已满。
    // 上限 = 厂等级；队列串行，已满 ≠ 产能瓶颈，只表示最近下单密。
    const allBusy = plants.length > 0 && busyPlants === plants.length;
    let queueState = 'free';   // free=有余量 | full=整城队列满 | none=未建军工厂
    if (plants.length === 0) queueState = 'none';
    else if (allBusy) {
      const queued = async (b) => {
        const r6 = await c.call(3006, p.u64(b.bid));
        return r6.raw ? (parse3006(r6.raw).training || []).length : 0;
      };
      if (await queued(plants[0]) >= trainQueueCap(plants[0].level)) {
        let cap = 0;
        let used = 0;
        for (const b of plants) {
          cap += trainQueueCap(b.level);
          used += await queued(b);
        }
        if (cap > 0 && used >= cap) {
          queueState = 'full';
          empireQueueFullCities += 1;
        }
      }
    }

    empirePlantsTotal += plants.length;
    empirePlantsActive += busyPlants;
  ...
  if (SHOW_ALL) {
    ...
    console.log(`  城池总数: ${allCities.length} 座    当前钻石: ${fmtNum(base.diamond_owned)} 钻    军工运转: ${empirePlantsActive}/${empirePlantsTotal} 厂在产` +
      (empireQueueFullCities > 0 ? `（其中 ${empireQueueFullCities} 座城队列已满）` : ''));
  ...
  if (SHOW_ALL || SHOW_MIL) {
    ...
    const rows = cityData.map((d) => {
      ...
      if (d.plants.length > 0) {
        if (d.busyPlants > 0) {
          ...
          queueDesc = `${d.activeQueues[0].name} ×${d.busyPlants}厂 (${fmtDur(maxRemain)})`;
        } else {
          queueDesc = '空闲中';
        }
      }
      return [
        ...
        d.plants.length > 0 ? `${d.busyPlants}/${d.plants.length}` : '--',
        d.plants.length === 0 ? '--'
          : d.queueState === 'full' ? '已满'
            : '未满',
        queueDesc,
      ];
    });
    const footer = [
      ...
      empireQueueFullCities > 0 ? `${empireQueueFullCities} 座城队列满` : '',
      '',
    ];
  ...
  if (ONLY_CITY) {
    ...
    console.log(`  军工厂: ${d.plants.length ? `${d.busyPlants}/${d.plants.length} 在产，${d.queueState === 'full' ? '队列已满（下次运行改用 3004 按厂平摊）' : '有余量'}` : '未建军工厂'}`);
    for (const q of d.activeQueues) {
      console.log(`    ▷ ${q.name} 队首剩余 ${fmtDur(q.remainMs)}（完整队列深度用 w2train --dry 查）`);
    }
    console.log('─'.repeat(60));
  }
  ...
  if (SHOW_ALL) {
    ...
    const idlePlants = cityData.filter((d) => d.plants.length > 0 && d.busyPlants === 0);
    if (idlePlants.length > 0) {
      ...
      if (empireQueueFullCities > 0) {
        // 队列满只是「下单密」，不代表产能瓶颈（队列串行、单笔即可吃满资源）；此处仅提示
        console.log(`    其中 ${empireQueueFullCities} 座城队列已满——` +
          `队列串行不额外增产能，脚本下次运行会改用 3004 按厂平摊（协议层模拟客户端行为）。`);
      }
    }
