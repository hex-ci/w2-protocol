 ...
 *   node scripts/w2transport.js --clean-route      忽略拓扑缓存，强制重算协作组与中心仓
 *   node scripts/w2transport.js --recompute-hubs   只强制重算中心仓（协作组仍走缓存）
 *   node scripts/w2transport.js --army 6           目标兵种（默认侦察机），D/底仓线随兵种算
...
import { DispatchKey, fetchInFlight } from '../lib/expedition.js';
import { createProgress } from '../lib/progress.js';
import { DEFAULT_ARMY_ID, requireArmy, armyLabel, armyUnit } from '../lib/army.js';
...
// 中心仓冻结：指纹不变即复用缓存仓选，不重算（避免实时产量波动导致仓址来回抖动）。
// 需要重新选址时显式加 --recompute-hubs。
const RECOMPUTE_HUBS = has('recompute-hubs');
...
// 目标兵种：D 需求层与底仓线都随兵种算（默认侦察机）
const ARMY_ID = parseInt(arg('army', String(DEFAULT_ARMY_ID)), 10) || DEFAULT_ARMY_ID;
// 计数单位随兵种变（侦察机「架」、卡车「辆」……），产需账本表头用它
...
(async function main() {
  ...
  // 兵种参数校验（非法 ID 直接报错并列出可用兵种，别让后续计算用错单价）
  requireArmy(ARMY_ID);
  ...
  if (cachedTopology) {
    ...
    // 拓扑重算 = 城址变了，中心仓随之重算（这是唯一该重算的情形）
    topoMsg = '【拓扑自愈】已重新计算地缘协作组与中心仓。';
  }

  // 进度行活跃期间的业务输出统一延迟到 done() 之后打印，避免与进度行串行
  const deferred = [];
  const frozen = [];   // 复用了缓存仓选的资源（用于输出说明）

  /**
   * 核心仓选址：**缓存优先（冻结）**。
   *
   * 为什么冻结：评分权重是实时量 max(0, P−D)，P 随资源产出波动、D 还随兵种变——
   * 实测候补之间常只差 3~5%（矿仓 2.98%、粮仓 3.47%、油仓 4.56%），
   * 每日重算会让仓址来回翻转，同一批货今天归集到 A、明天到 B，运输策略失去意义。
   * 城址（空间指纹）才是选址的稳定依据：指纹不变即复用，迁城/增减城才失效重算。
   * 需要按最新产量重新选址时用 --recompute-hubs。
   */
  function resolveHub(resKey, override) {
    ...
    // 冻结命中：拓扑指纹一致（cachedTopology 非空即已校验）且缓存里有该资源的仓 → 直接复用
    if (!RECOMPUTE_HUBS && cachedTopology?.superHubs) {
      const cachedId = cachedTopology.superHubs[resKey];
      if (cachedId) {
        const hit = state.find((s) => s.cityId === String(cachedId));
        if (hit) {
          frozen.push(`${resKey}=${hit.name}`);
          return hit;
        }
      }
    }
  ...
  if (frozen.length && !RECOMPUTE_HUBS) {
    console.log(`【中心仓冻结】城址未变，复用上次选址（${frozen.length}/5 项）。`
      + `需按最新产量重新选址时加 --recompute-hubs。`);
  } else if (RECOMPUTE_HUBS) {
    console.log('【中心仓重算】--recompute-hubs：按当前产量重新选址。');
  }
  ...
  if (PROFILE) {
    ...
    const columns = [
      ...
      { header: `产能${UNIT}/h`, width: 8, align: 'right' },
      { header: `自持${UNIT}/h`, width: 8, align: 'right' },
  ...
  console.log(`模式: ${DRY ? '【模拟运算 (DRY RUN)】' : '【正式执行下单】'} | 兵种: ${armyLabel(ARMY_ID)} | 底仓线: ${FLOOR_HOURS}h（按该兵种造价计） | 黄金超容阈值: ${Math.round(OVERFLOW_PCT * 100)}%`);
  ...
  for (let i = 0; i < tasks.length; i++) {
    ...
    if (!DRY) {
      ...
      if (resp.ok) {
        ...
        // 服务端对多种拒绝原因返回同一状态码，不判断文案：重查 19008 在途数——
        // 若已达上限说明是配额问题，标记该城跳过剩余任务；否则是资源/其它原因，仅报告。
        const inflight = await fetchInFlight(c);
        const atSrc = inflight.filter((f) => f.startX === t.src.x && f.startY === t.src.y).length;
        if (t.src.slotsCap !== Infinity && atSrc >= t.src.slotsCap) {
          t.src.slotsUsed = t.src.slotsCap;
          console.log(`       ✗ [19003 失败] 出征位已满（在途 ${atSrc}/${t.src.slotsCap}），该城剩余任务自动跳过`);
        } else {
