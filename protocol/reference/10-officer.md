# 名将

> 42 个命令（cmd 11001 ~ 11059）。所有响应均以 1 字节 status 打头，成功值见各条目。

### `cmd=11001` — 查询可招募名将

**请求参数**: 无

**响应**（status 为 **1** 时成功；失败时仅 1 字节状态 + 错误文案字符串）:

> 含列表：先读计数字段，再按下列顺序循环读取每个条目。

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u32 | `remain_slots` | — |
| 2 | string | `free_refresh_name` | 名称 |
| 3 | u32 | `free_refresh_icon` | 图标编号 |
| 4 | string | `free_refresh_description` | 描述文案 |
| 5 | u32 | `free_refresh_count` | 数量/计数 |
| 6 | u32 | `free_refresh_count_max` | 数量/计数 |
| 7 | u64 | `officer_id` | — |
| 8 | string | `officer_name` | 名称 |
| 9 | u32 | `icon` | 图标编号 |
| 10 | u32 | `level` | 等级 |
| 11 | u32 | `star` | — |
| 12 | u32 | `logistics_base` | — |
| 13 | u32 | `military_base` | — |
| 14 | u32 | `knowledge_base` | — |
| 15 | u32 | `faithful` | — |
| 16 | u32 | `salary` | — |
| 17 | u32 | `gold_required` | — |
| 18 | u32 | `is_special_officer` | 布尔标记（0/1） |

---

### `cmd=11002` — 查询名将详情

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |

**响应**（status 为 **1** 时成功；失败时仅 1 字节状态 + 错误文案字符串）:

> 含列表：先读计数字段，再按下列顺序循环读取每个条目。

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |
| 2 | u32 | `prototype_id` | 建筑原型 ID |
| 3 | string | `officer_name` | 名称 |
| 4 | u32 | `icon` | 图标编号 |
| 5 | u32 | `level` | 等级 |
| 6 | u32 | `star` | — |
| 7 | u32 | `experience` | — |
| 8 | u32 | `experience_next_level` | 等级 |
| 9 | u32 | `logistics_base` | — |
| 10 | u32 | `military_base` | — |
| 11 | u32 | `knowledge_base` | — |
| 12 | u32 | `faithful` | — |
| 13 | u32 | `salary` | — |
| 14 | u32 | `potential` | — |
| 15 | u32 | `attack` | — |
| 16 | u32 | `defence` | — |
| 17 | u32 | `leader_ship` | — |
| 18 | u32 | `skill_id` | — |
| 19 | string | `icon` | 图标编号 |
| 20 | string | `skill_name` | 名称 |
| 21 | u32 | `skill_level` | 等级 |
| 22 | string | `skill_description` | 描述文案 |
| 23 | u64 | `equipment_id` | — |
| 24 | string | `name` | 名称 |
| 25 | u32 | `icon` | 图标编号 |
| 26 | u32 | `position` | 格位编号 |
| 27 | u8 | `level` | 等级 |
| 28 | u32 | `level_required_on_wear` | 等级 |
| 29 | u32 | `military` | — |
| 30 | u32 | `knowledge` | — |
| 31 | u32 | `logistics` | — |
| 32 | u32 | `attack` | — |
| 33 | u32 | `defence` | — |
| 34 | u32 | `is_bind` | 布尔标记（0/1） |
| 35 | string | `description` | 描述文案 |
| 36 | u8 | `cur_endure` | 当前值 |
| 37 | u8 | `max_endure` | 上限 |
| 38 | u32 | `gold_required_on_repair` | — |
| 39 | u32 | `promotion_item_count` | 数量/计数 |
| 40 | u16 | `logistics_add` | — |
| 41 | u16 | `military_add` | — |
| 42 | u16 | `knowledge_add` | — |
| 43 | u16 | `attack_add` | — |
| 44 | u16 | `defence_add` | — |
| 45 | u16 | `leader_ship_add` | — |
| 46 | u8 | `is_officer_troop` | 布尔标记（0/1） |
| 47 | u8 | `is_sackable` | 布尔标记（0/1） |
| 48 | u32 | `item_id` | 道具 ID |
| 49 | string | `item_name` | 道具名称 |
| 50 | u8 | `talent_value` | — |
| 51 | u32 | `talent_add_by_cimelia` | — |

---

### `cmd=11003` — 免费刷新名将招募

**请求参数**: 无

**响应**: 无业务数据。status 为 **1** 时成功；失败时为状态字节 + 错误文案。

---

### `cmd=11004` — 招募名将

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |

**响应**（status 为 **1** 时成功；失败时仅 1 字节状态 + 错误文案字符串）:

> 含列表：先读计数字段，再按下列顺序循环读取每个条目。

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |
| 2 | string | `officer_name` | 名称 |
| 3 | u32 | `icon` | 图标编号 |
| 4 | u32 | `level` | 等级 |
| 5 | u32 | `status` | 结果状态 |

---

### `cmd=11007` — 名将升级

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |
| 2 | u8 | `auto_upgrade` | — |

**响应**（status 为 **1** 时成功；失败时仅 1 字节状态 + 错误文案字符串）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |
| 2 | u32 | `level` | 等级 |
| 3 | u32 | `experience` | — |
| 4 | u32 | `experience_next_level` | 等级 |
| 5 | u32 | `potential` | — |
| 6 | u32 | `salary` | — |

---

### `cmd=11008` — 名将洗练

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |
| 2 | u8 | `confirm_using_diamond` | — |

**响应**（status 为 **1** 时成功；失败时仅 1 字节状态 + 错误文案字符串）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | string | `message` | — |

---

### `cmd=11009` — 名将潜力分配

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |
| 2 | u32 | `knowledge_add` | — |
| 3 | u32 | `military_add` | — |
| 4 | u32 | `logistics_add` | — |

**响应**: 无业务数据。status 为 **1** 时成功；失败时为状态字节 + 错误文案。

---

### `cmd=11010` — 任命市长

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |

**响应**: 无业务数据。status 为 **1** 时成功；失败时为状态字节 + 错误文案。

---

### `cmd=11011` — 名将罢免

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |

**响应**: 无业务数据。status 为 **1** 时成功；失败时为状态字节 + 错误文案。

---

### `cmd=11012` — 名将解雇

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |

**响应**: 无业务数据。status 为 **1** 时成功；失败时为状态字节 + 错误文案。

---

### `cmd=11013` — 释放被俘名将

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |

**响应**（status 为 **1** 时成功；失败时仅 1 字节状态 + 错误文案字符串）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |

---

### `cmd=11014` — 劝降被俘名将

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |

**响应**（status 为 **1** 时成功；失败时仅 1 字节状态 + 错误文案字符串）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |
| 2 | u8 | `officer_status` | 结果状态 |

---

### `cmd=11015` — 处决被俘名将

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |

**响应**（status 为 **1** 时成功；失败时仅 1 字节状态 + 错误文案字符串）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |
| 2 | string | `officer_said` | — |

---

### `cmd=11018` — 名将晋升

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |
| 2 | u8 | `confirm_using_diamond` | — |

**响应**（status 为 **1** 时成功；失败时仅 1 字节状态 + 错误文案字符串）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | string | `message` | — |
| 2 | u64 | `officer_id` | — |
| 3 | string | `message` | — |

---

### `cmd=11019` — 遗忘名将技能

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |
| 2 | u32 | `skill_id` | — |

**响应**: 无业务数据。status 为 **1** 时成功；失败时为状态字节 + 错误文案。

---

### `cmd=11020` — 查询可授予道具

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |

**响应**（status 为 **1** 时成功；失败时仅 1 字节状态 + 错误文案字符串）:

> 含列表：先读计数字段，再按下列顺序循环读取每个条目。

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u32 | `gold_award` | — |
| 2 | u32 | `item_id` | 道具 ID |
| 3 | u32 | `icon` | 图标编号 |
| 4 | string | `name` | 名称 |
| 5 | string | `description` | 描述文案 |
| 6 | u32 | `amount` | 数量 |

---

### `cmd=11021` — 授予名将黄金

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |

**响应**: 无业务数据。status 为 **1** 时成功；失败时为状态字节 + 错误文案。

---

### `cmd=11022` — 穿戴装备

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |
| 2 | u64 | `equipment_id` | — |

**响应**: 无业务数据。status 为 **1** 时成功；失败时为状态字节 + 错误文案。

---

### `cmd=11023` — 卸下装备

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |
| 2 | u64 | `equipment_id` | — |

**响应**: 无业务数据。status 为 **1** 时成功；失败时为状态字节 + 错误文案。

---

### `cmd=11024` — 卸下全部装备

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |

**响应**: 无业务数据。status 为 **1** 时成功；失败时为状态字节 + 错误文案。

---

### `cmd=11027` — 查询城内名将

> **军官跨城迁移机制（实测）**：军官随部队走——用 `cmd=19003` 派遣（`expedition_type=4` DISPATCH）
> 到目标城，`commander_id` 填军官 ID 即可；兵种任选（侦察机速度 5000 最快，1 架就够，跨城数分钟到达）。
> 派遣中该军官在本城 `11027` 列表里 `status=1`，到达目标城后从出发城列表消失、出现在目标城列表。
> 军官的 `salary` 俸禄由**所在城**的黄金税收支付（2027 的 `gold_officer_used`）；军官过于集中会使该城黄金
> 净产为负，可用派遣分摊到他城使各城净产回正。派遣本身只占用 1 个出征位与 1 单位载具，不携带货物。

**请求参数**: 无

**响应**（status 为 **1** 时成功；失败时仅 1 字节状态 + 错误文案字符串）:

> 含列表：先读计数字段，再按下列顺序循环读取每个条目。

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |
| 2 | string | `officer_name` | 名称 |
| 3 | u32 | `icon` | 图标编号 |
| 4 | u32 | `level` | 等级 |
| 5 | u32 | `star` | — |
| 6 | u32 | `logistics_base` | — |
| 7 | u32 | `military_base` | — |
| 8 | u32 | `knowledge_base` | — |
| 9 | u32 | `faithful` | — |
| 10 | u32 | `salary` | 每小时俸禄（黄金） |
| 11 | u32 | `status` | 状态：0=在城待命、1=派遣在途、2=任市长 |
| 12 | u32 | `attack` | — |
| 13 | u32 | `defence` | — |
| 14 | u32 | `prototype_id` | 建筑原型 ID |
| 15 | u32 | `leader_ship` | — |
| 16 | u16 | `logistics_add` | — |
| 17 | u16 | `military_add` | — |
| 18 | u16 | `knowledge_add` | — |
| 19 | u32 | `skill_oil_add` | — |
| 20 | u32 | `logistics_item_add` | 道具 |
| 21 | u32 | `military_item_add` | 道具 |
| 22 | u32 | `knowledge_item_add` | 道具 |
| 23 | u32 | `logistics_without_item_and_troop` | 道具 |
| 24 | u32 | `military_without_item_and_troop` | 道具 |
| 25 | u32 | `knowledge_without_item_and_troop` | 道具 |
| 26 | u8 | `is_officer_troop` | 布尔标记（0/1） |
| 27 | u32 | `officer_troop_id` | — |
| 28 | u8 | `is_main_officer` | 布尔标记（0/1） |
| 29 | u64 | `main_officer_id` | — |
| 30 | u32 | `leader_ship_addition_percent` | — |
| 31 | u32 | `talent_add_by_cimelia` | — |

---

### `cmd=11031` — 招募名将（高级）

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u32 | `item_id` | 道具 ID |

**响应**: 无业务数据。status 为 **1** 时成功；失败时为状态字节 + 错误文案。

---

### `cmd=11032` — 招募名将（普通）

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u32 | `item_id` | 道具 ID |

**响应**（status 为 **1** 时成功；失败时仅 1 字节状态 + 错误文案字符串）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | string | `message` | — |

---

### `cmd=11034` — 学习技能

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |
| 2 | u32 | `item_id` | 道具 ID |

**响应**（status 为 **1** 时成功；失败时仅 1 字节状态 + 错误文案字符串）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | string | `message` | — |

---

### `cmd=11035` — 增加名将经验

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u32 | `item_id` | 道具 ID |
| 2 | u32 | `amount` | 数量 |
| 3 | u64 | `officer_id` | — |

**响应**: 无业务数据。status 为 **1** 时成功；失败时为状态字节 + 错误文案。

---

### `cmd=11036` — 授予名将道具

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u32 | `item_id` | 道具 ID |
| 2 | u64 | `officer_id` | — |

**响应**: 无业务数据。status 为 **1** 时成功；失败时为状态字节 + 错误文案。

---

### `cmd=11038` — 创建名将部队

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `leader` | — |
| 2 | u32 | `aides.length` | — |
| 3 | 循环 | — | 按前导计数字段循环写入后续字段 |

**响应**: 无业务数据。status 为 **1** 时成功；失败时为状态字节 + 错误文案。

---

### `cmd=11039` — 统帅计算参数

**请求参数**: 无

**响应**（status 为 **1** 时成功；失败时仅 1 字节状态 + 错误文案字符串）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u32 | `military_percent` | — |
| 2 | u32 | `knowledge_percent` | — |
| 3 | u32 | `logistics_percent` | — |
| 4 | u32 | `first_percent` | — |
| 5 | u32 | `second_percent` | — |
| 6 | u32 | `third_percent` | — |
| 7 | u32 | `military_state` | — |
| 8 | u32 | `knowledge_state` | — |
| 9 | u32 | `logistics_state` | — |
| 10 | u32 | `military_pri` | — |
| 11 | u32 | `knowledge_pri` | — |
| 12 | u32 | `logistics_pri` | — |

---

### `cmd=11040` — 更新名将部队

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u32 | `troop_id` | — |
| 2 | u64 | `leader` | — |
| 3 | u32 | `aides.length` | — |
| 4 | 循环 | — | 按前导计数字段循环写入后续字段 |

**响应**: 无业务数据。status 为 **1** 时成功；失败时为状态字节 + 错误文案。

---

### `cmd=11041` — 解散名将部队

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u32 | `troop_id` | — |

**响应**: 无业务数据。status 为 **1** 时成功；失败时为状态字节 + 错误文案。

---

### `cmd=11042` — 查询名将状态

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |

**响应**（status 为 **1** 时成功；失败时仅 1 字节状态 + 错误文案字符串）:

> 含列表：先读计数字段，再按下列顺序循环读取每个条目。

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u32 | `attr_type` | 类型枚举 |
| 2 | u32 | `percent_value` | — |
| 3 | string | `item_name` | 道具名称 |
| 4 | u64 | `remain_time` | 剩余毫秒数 |

---

### `cmd=11043` — 名将强化

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u32 | `item_id` | 道具 ID |
| 2 | u64 | `officer_id` | — |

**响应**: 无业务数据。status 为 **1** 时成功；失败时为状态字节 + 错误文案。

---

### `cmd=11044` — 查询可穿戴装备

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u32 | `equipment_position` | 格位编号 |
| 2 | u64 | `officer_id` | — |
| 3 | u8 | `page_size` | 每页条数 |
| 4 | u32 | `page_num` | 页码（从 0 或 1 起，随接口） |
| 5 | string | `keyword` | — |

**响应**（status 为 **1** 时成功；失败时仅 1 字节状态 + 错误文案字符串）:

> 含列表：先读计数字段，再按下列顺序循环读取每个条目。

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u32 | `page_num` | 页码（从 0 或 1 起，随接口） |
| 2 | u32 | `page_count` | 总页数 |
| 3 | u64 | `equipment_id` | — |
| 4 | string | `name` | 名称 |
| 5 | u32 | `icon` | 图标编号 |
| 6 | u8 | `level` | 等级 |
| 7 | u32 | `military` | — |
| 8 | u32 | `knowledge` | — |
| 9 | u32 | `logistics` | — |
| 10 | u32 | `attack` | — |
| 11 | u32 | `defence` | — |
| 12 | u32 | `level_required_on_wear` | 等级 |
| 13 | u8 | `is_weared` | 布尔标记（0/1） |
| 14 | u8 | `is_suit_equipment` | 布尔标记（0/1） |

---

### `cmd=11045` — 查询部队技能原型

**请求参数**: 无

**响应**（status 为 **1** 时成功；失败时仅 1 字节状态 + 错误文案字符串）:

> 含列表：先读计数字段，再按下列顺序循环读取每个条目。

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u32 | `troop_skill_id` | — |
| 2 | string | `icon` | 图标编号 |
| 3 | string | `skill_name` | 名称 |
| 4 | u32 | `skill_level` | 等级 |
| 5 | string | `skill_level_description` | 描述文案 |
| 6 | string | `skill_level_bonus_description` | 描述文案 |
| 7 | u32 | `logistics_add` | — |
| 8 | u32 | `military_add` | — |
| 9 | u32 | `knowledge_add` | — |
| 10 | u64 | `prototype_id` | 建筑原型 ID |
| 11 | u32 | `icon` | 图标编号 |
| 12 | string | `officer_name` | 名称 |
| 13 | u32 | `already_owned` | — |

---

### `cmd=11046` — 名将驻防

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |

**响应**（status 为 **1** 时成功；失败时仅 1 字节状态 + 错误文案字符串）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |

---

### `cmd=11047` — 名将撤防

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |

**响应**: 无业务数据。status 为 **1** 时成功；失败时为状态字节 + 错误文案。

---

### `cmd=11048` — 道具刷新招募

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u32 | `item_id` | 道具 ID |

**响应**: 无业务数据。status 为 **1** 时成功；失败时为状态字节 + 错误文案。

---

### `cmd=11049` — 查询可穿戴套装

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |

**响应**（status 为 **1** 时成功；失败时仅 1 字节状态 + 错误文案字符串）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u32 | `suit_id` | — |
| 2 | string | `name` | 名称 |
| 3 | u32 | `level_required_on_wear` | 等级 |

---

### `cmd=11050` — 穿戴套装

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |
| 2 | u32 | `suit_id` | — |

**响应**: 无业务数据。status 为 **1** 时成功；失败时为状态字节 + 错误文案。

---

### `cmd=11057` — 查询晋阶信息

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |

**响应**（status 为 **1** 时成功；失败时仅 1 字节状态 + 错误文案字符串）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |
| 2 | u32 | `promote_available` | — |
| 3 | u32 | `level_from` | 等级 |
| 4 | u32 | `level_to` | 等级 |
| 5 | u32 | `extra_talent` | — |
| 6 | string | `new_name` | 名称 |
| 7 | string | `detail` | — |
| 8 | u32 | `item_id` | 道具 ID |
| 9 | string | `item_name` | 道具名称 |
| 10 | u32 | `need_count` | 数量/计数 |
| 11 | u32 | `have_count` | 数量/计数 |

---

### `cmd=11058` — 名将晋阶

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |

**响应**（status 为 **1** 时成功；失败时仅 1 字节状态 + 错误文案字符串）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |
| 2 | string | `officer_name` | 名称 |
| 3 | u32 | `level` | 等级 |
| 4 | u32 | `talent` | — |

---

### `cmd=11059` — 增加潜力点

**请求参数**（按序拼接为 AES 明文）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u32 | `item_id` | 道具 ID |
| 2 | u64 | `officer_id` | — |

**响应**（status 为 **1** 时成功；失败时仅 1 字节状态 + 错误文案字符串）:

| 顺序 | 类型 | 字段 | 说明 |
|---|---|---|---|
| 1 | u64 | `officer_id` | — |
| 2 | u32 | `talent` | — |
| 3 | u32 | `talent_add_by_cimelia` | — |
