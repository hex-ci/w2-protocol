...
> 每种资源块为 24B；储量在块内 +0、容量 +4（复核：粮/钢/矿/油四块容量确认，黄金/人口块未逐项验证）。
...
**产量结算机制（tick 实测 + 精确公式复核）**：

- 服务器资源**不是连续增长，而是按 tick 结算**：实测约 60 秒一跳，两次采样之间增量为 0，到点一次性跳变。
- 精确公式（4 城交叉验证偏差 <0.3%）：
  ```
  每 tick 产量 = basic_output × (nature + tech + officer) / 100 ÷ 60
  即 实收/小时 = basic_output × (nature + tech + officer) / 100
  ```
  其中 nature/tech/officer 取自 `cmd=2018` 产量加成（nature 与 tech 常见值 100，officer 随城 35~110）。
  观测示例：XXX城 basic=49005、加成 100/100/88 → 实收 ≈ 141,134/h，单 tick 实测 2354（预测 2352.2）。
  简化口径（默认 nature/tech 均为 100 时）：**实收/h ≈ basic_output × 2.88**。
- 资源达容量上限后停止增长；超上限部分（运输造成）不再自然增长。
