...
- **登录参数**：`platform`、`channel`、`language` 由渠道分发决定，须与所用渠道一致（渠道不匹配会导致登录失败）；`clientVer` 同理。部署侧取值见 `.env.example` 的 `W2_LOGIN_*`（无代码内置缺省，必须配置）。
...
- **请求时序**：真机初始化阶段按「批」突发多帧（如 no=7 一次并发 7 个查询）；模拟端按 500ms 起随机间隔逐帧发送（SDK 内置 0~50% 抖动），节奏更平缓。
...
7. **频控**：客户端对同命令 500ms 内去重；SDK 串行发送并在 500ms 基础上叠加 0~50% 随机抖动。服务器侧未见同类限制。
8. **训练队列上限 = 厂等级**：军工厂训练队列每厂可排 `厂等级` 条（实测 L9=9 / L10=10），超出上限的订单被服务端拒绝。
   队列**串行**——只有队首条目消耗时间，其余条目 `remain_time` 静止，提交后总工期 = 各条 remain 之和。
   队列上限不构成产能上限：**单笔 3001 即可吃满全城资源上限**（实测单笔 15k+ 架成功），
   故分散下单的收益来自「覆盖多厂并行」，而非堆积队列条数。
   `cmd=3004` 把总量平摊到该城所有队列未满的厂（跳过队列已满的厂），不参考各厂队列长短；
   全城队列满时与其它拒绝返回同一失败状态。
...
```
① SSO 账号登录   POST mlogin → WST(凭据) + WTGT(续登票据) + 账号标识
...
- **端点**：`W2_SSO_URL` 下的 `mlogin` 表单（无代码内置缺省，必须在本地 `.env` 配置；工具拒绝 HTTP）。
...
- **响应关键字段**：
  ...
  - **账号标识字段**（64 hex，键名由服务端下发，含厂商词根故不在仓库中出现；运行期键名见 `.env` 的 `W2_SSO_USERID_FIELD`）——DES 解密即 `username`（36 字符 UUID），与抓包 1001 帧的 username 三方一致。
...
- **独立于游戏服**：两端各自独立部署（地址与端口由本地 `.env` 的 `W2_SSO_CHOICE_HOST` / `W2_SSO_CHOICE_PORT` 配置）。iOS 走裸 TCP 明文 WIST 帧；Android 走 **WebSocket**（裸 TCP 发 WIST 帧无响应）。**按平台隔离部署，服务端校验 platform/channel 匹配**——平台不匹配时返回 `status=-1` 并附错误文案。
