...
| `3004` ✦ | 训练并自动分城 | `long buildingId` + `int armyId` + `int amount`（总量平摊到有空槽厂） |
...
- `attachmentFlag` 是**三态**：`255` 无附件 / `0` 有待领附件 / `1` 已领取（9008 成功后 `0`→`1`）。当布尔用会取反语义、整批漏掉待领邮件；可领性以 9002 详情附件条目非空为准，详见 reference/08-mail.md。
