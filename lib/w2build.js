...
function requireU32(value, name) {
  if (!Number.isInteger(value) || value < 0 || value > 0xffffffff) {
    throw new RangeError(`${name} 必须是 0 ~ 4294967295 的整数`);
  }
  return value;
}

function requireByte(value, name) {
  const parsed = requireU32(value, name);
  if (parsed > 0xff) throw new RangeError(`${name} 必须是 0 ~ 255 的整数`);
  return parsed;
}

function requireU64(value, name) {
  if (typeof value === 'number' && !Number.isSafeInteger(value)) {
    throw new RangeError(`${name} 的 number 值必须是安全整数；大数请传十进制字符串或 BigInt`);
  }
  let parsed;
  try { parsed = BigInt(value); } catch { throw new RangeError(`${name} 必须是无符号 64 位整数`); }
  if (parsed < 0n || parsed > 0xffffffffffffffffn) {
    throw new RangeError(`${name} 必须是无符号 64 位整数`);
  }
  return parsed;
}
...
function aesKey(sessionId) {
  return Buffer.from(String(requireU32(sessionId, 'sessionId')).padStart(16, '0'), 'latin1');
}
...
function buildFrame(no, sessionId, cmd, params = Buffer.alloc(0)) {
  requireU32(no, 'no');
  requireU32(sessionId, 'sessionId');
  requireU32(cmd, 'cmd');
...
const p = {
  byte: (v) => { const b = Buffer.alloc(1); b[0] = requireByte(v, 'byte'); return b; },
  u32: (v) => { const b = Buffer.alloc(4); b.writeUInt32BE(requireU32(v, 'u32'), 0); return b; },
  u64: (v) => { const b = Buffer.alloc(8); b.writeBigUInt64BE(requireU64(v, 'u64'), 0); return b; },
  str: (s) => {
    if (typeof s !== 'string') throw new TypeError('str 必须是字符串');
  ...
  cat: (...parts) => {
    if (!parts.every(Buffer.isBuffer)) throw new TypeError('cat 参数必须都是 Buffer');
...
function parseFrame(raw) {
  if (!Buffer.isBuffer(raw) || raw.length < 37 || raw.toString('latin1', 0, 4) !== 'WiST' || raw[4] !== 2) return null;
  const len = raw.readUInt32BE(29);
  if (len < 20 || 33 + len !== raw.length) return null;
  return {
...
function selfTest() {
  ...
  const cases = [
    ...
    ['cat', p.cat(p.u64('7000000000001234'), p.str('ios'), p.str('zh')), 8 + 4 + 3 + 4 + 2],
  ];
  const sid = 884422;
  ...
  try { p.u32(Number.NaN); fail += 1; console.log('  FAIL reject invalid u32'); } catch { console.log('  OK  reject invalid u32'); }
  try { p.byte(256); fail += 1; console.log('  FAIL reject invalid byte'); } catch { console.log('  OK  reject invalid byte'); }
  try { p.u64(-1); fail += 1; console.log('  FAIL reject invalid u64'); } catch { console.log('  OK  reject invalid u64'); }
  try { p.u64(Number.MAX_SAFE_INTEGER + 1); fail += 1; console.log('  FAIL reject unsafe number u64'); } catch { console.log('  OK  reject unsafe number u64'); }
