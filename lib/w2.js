...
/**
 * w2.js —— 私有 TCP 协议解析库（零依赖，Node 18+ / OpenWrt nodejs v20 可跑）
 *
 * 协议格式（2026-09-07 逆向所得）
 *
 *  客户端 -> 服务器   魔数 "WiST"，变长帧
 *    +0   4  magic
 *    +4   1  0x02
 *    +5   4  u32 序号（每帧 +1）
 *    +9   16 nonce
 *    +25  4  u32 fieldA（登录阶段为具体 ID，稳态恒 0xffffffff）
 *    +29  4  u32 长度 L
 *    +33  L  payload → 首 u32 = 命令字
 *              L=20  → [命令字][参数 16B]
 *              L=228 → [cmd=1001][224B 加密凭据]
 *
 *  服务器 -> 客户端   魔数 "WIST"
 *    +0   4  magic
 *    +4   4  u32 fieldA（响应类型；0xffffffff 表示服务器主动推送）
 *    +8   4  u32 长度
 *    +12  ... body（4 字节长度前缀 UTF-8 字符串 + 整数混编）
 */
