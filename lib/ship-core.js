...
/**
 * 列表选择的循环索引：末行继续向下回到首行，首行向上跳到末行。
 * @param {number} cur 当前下标
 * @param {number} delta 步进（-1 上 / +1 下）
 * @param {number} len 列表长度（0 时恒为 0，避免取模得 NaN）
 */
function wrapIndex(cur, delta, len) {
  if (len <= 0) return 0;
...
function cityLimits(src, dst, fuelHours = DEFAULT_FUEL_HOURS) {
  ...
  else if (!RES_ALL.some((r) => (src.stock[r] || 0) > 0)) blocker = 'stock';
 ...
 * 黄金无造兵需求、底仓线不存在（floor 里没有 gold），该口径对它无意义 → 返回 0。
 */
function presetFill(src, dst, res, loads, fuelHours) {
  if ((dst.floor || {})[res] == null) return 0;
  const need = Math.max(0, dst.floor[res] - (dst.stock[res] || 0));
  return clampAmount(src, dst, res, need, loads, fuelHours);
}

/**
 * y 运出全部富余：源城高于底仓线的部分。
 * 同 presetFill——黄金没有底仓线，不能把「底仓线缺失」当成 0 而判为「全部富余」
 * （那会把整城黄金一次性运走）。要外运黄金请用 e（超容口径）。
 */
function presetSurplus(src, dst, res, loads, fuelHours) {
  if ((src.floor || {})[res] == null) return 0;
  const surplus = Math.max(0, (src.stock[res] || 0) - src.floor[res]);
