...
// 登录参数缺省值：渠道与版本随发布方不同，没有通用真值——
// 真值由本地 .env 的 W2_LOGIN_CHANNEL / W2_LOGIN_CLIENT_VER 提供（.env 不入库）。
const DEFAULT_CLIENT_VER = 0;
const DEFAULT_PLATFORM = 'ios';
const DEFAULT_CHANNEL = '';
const DEFAULT_LANGUAGE = 'zh';
const MAX_FRAME_BODY = 1048576;
// 帧间隔抖动上限（比例）
const INTER_REQUEST_JITTER = 0.5;
...
function clientUserAgent() {
  // 缺省为通用移动端 UA；真机 UA 形态由本地 .env 的 W2_LOGIN_UA 提供，
  // 模板中的 {clientVer} 占位符与登录帧版本号同源
  const tpl = process.env.W2_LOGIN_UA || (config.fileEnv && config.fileEnv.W2_LOGIN_UA);
  if (tpl) return tpl.replace('{clientVer}', String(clientVer()));
...
class W2Client {
   ...
   *   login               登录帧 hex 直发（调试用，loginParams 缺省时的回退）
   *   timeout             单请求超时 ms，默认 10000
   *   interRequest        帧间基础间隔 ms，默认 500；实际间隔 = 基础值 + 0~50% 随机抖动
  ...
  constructor(opts = {}) {
    ...
    this.loginFrameHex = opts.login || ''; // 帧直发（调试用，正式凭据走 loginParams）
   ...
   *      凭据经 scripts/w2login.js 登录后持久化到 .identity.local.json，由 config.loginParams() 读取）
   *   2. login 帧 hex 直发 —— 无 loginParams 时的回退路径
  ...
  // ---------- 内部 ----------
  /**
   * 帧间节流：基础间隔 + 0~50% 均匀随机抖动。
   */
  async _throttle() {
    const reserve = async () => {
      const interval = this.interRequest * (1 + Math.random() * INTER_REQUEST_JITTER);
      const wait = this._lastSendAt + interval - Date.now();
