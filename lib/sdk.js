...
// 客户端版本整数默认值（登录字段之一，服务端不作强校验）
const DEFAULT_CLIENT_VER = 3036900;
const { buildFrame, p } = require('./w2build.js');
...
class W2Client {
  ...
  async _login(opts = {}) {
    ...
    if (lp && lp.wst) {
      // 完整登录：按 NOTES §7 的 1001 明文结构逐字段构造
      const params = p.cat(
        p.u64(lp.userId || 0),
        p.str(lp.username || ''),
        p.u32(lp.clientVer || DEFAULT_CLIENT_VER),
        p.str(lp.platform || 'ios'),
        p.str(lp.channel || ''),
        p.str(lp.language || 'zh'),
        p.str(lp.appKey || ''),
        p.str(lp.wst),
        p.str(lp.installID || '')
      );
      this.no += 1;
      const sid = 100000 + Math.floor(Math.random() * 1000000);
  ...
  async call(cmd, params = Buffer.alloc(0), schema = null, opts = {}) {
    ...
    this.no += 1;
    const sid = 100000 + Math.floor(Math.random() * 1000000); // 每帧独立随机
...
module.exports = { W2Client, decodeBySchema, p, sleep };
