...
function clientVer() {
  const v = parseInt(process.env.W2_LOGIN_CLIENT_VER || '', 10);
  if (v) return v;
  const fromEnvFile = parseInt((config.fileEnv && config.fileEnv.W2_LOGIN_CLIENT_VER) || '', 10);
  return fromEnvFile || DEFAULT_CLIENT_VER;
}
...
class W2Client {
   ...
   */
  async _login(opts = {}) {
    if (this.mode === 'choice') return { mode: 'skipped' }; // 选服服务器无登录概念
    const lp = opts.loginParams || this.loginParams;
    if (lp && lp.wst) {
      // 完整登录：按 NOTES §4 的 1001 明文结构逐字段构造
      // platform/channel/language/clientVer 缺省用 iOS 真机值
      // ⚠️ userId 必须与 wst 匹配：实测 userId=0/错值 + 有效 wst 触发风控（返回封停文案）
      // installID/appKey 由 loginParams 提供（登录命令生成并缓存），此处缺省随机值仅为兜底
      const appKey = lp.appKey || crypto.randomBytes(16).toString('hex');
      const installID = lp.installID || crypto.randomBytes(16).toString('hex').toUpperCase();
      const params = p.cat(
        p.u64(BigInt(lp.userId || 0)),
        p.str(lp.username || ''),
        p.u32(lp.clientVer || clientVer()),
  ...
  async call(cmd, params = Buffer.alloc(0), schema = null, opts = {}) {
    ...
    this.no += 1;
    const sid = this._nextSessionId();
    // 选服服务器帧为明文（无 AES/MD5）：WIST + sessionId + L(cmd+payload) + cmd + payload
    // 游戏服帧为加密（buildFrame）：WiST 0x02 + no + md5 + sessionId + len + cmd + AES(params)
