 ...
 *   cost（目标兵种单价）、armyId（目标兵种）、Cin（满负荷产能 件/h）、plants、special（主产资源）、
...
import {
  RES_KEYS,
  prodRate, surplusOf, deficitOf, expeditionSlots,
  armyCin, armyDemand, armyFloor, trainQueueCap,
} from './formula.js';
import { findArmy, DEFAULT_ARMY_ID } from './army.js';
 ...
 *   armyId      目标兵种 ID（默认侦察机）：决定 D 需求层与底仓线——不同兵种的
 *               单价/耗时不同，D 与 floor 必须按兵种算（实测侦察机与重坦的粮需求差 7 倍）
 *   onCities    城池列表拉取后的回调 (cities)，用于拓扑检查等前置工作
 *   onCity      每城完成回调 (state_i, index, total)，用于进度展示
 *   onStage     阶段切换回调 (label)，调用方据此更新进度行
 *   onWarn      扫描中的跳城/跳厂告警 (message)，进度行活跃期间不可直接打印，
 *               调用方自行决定先缓存待 done() 后输出
 * @returns {Promise<{cities, state, originCityId, bonus19001}>}
 *   bonus19001 = { landSpeedAdd, speedFactor }（行军实时加成，19001 下发）
 */
async function scanDomain(c, { floorHours = 24, armyId = DEFAULT_ARMY_ID, onCities = null, onCity = null, onStage = null, onWarn = null } = {}) {
  ...
  const state = [];
  // 每城 6~7 个请求 × 帧间隔（500ms 起随机抖动），是首屏延迟的主要来源：按城推进反馈
  // 总数经 onStage 的 meta 暴露；onCity 只在每城完成时触发（消费方会读 state 字段）
  stage('扫描城池', { total: cities.length });
  for (let i = 0; i < cities.length; i++) {
    const city = cities[i];
    // 切城失败必须报错：后续读取会静默拿到上一座城的数据，污染产需与调度计算
    const sw = await c.call(2002, p.u64(city.cityId));
    if (!sw.ok) throw new Error(`切城失败（cmd=2002，城池 ${city.cityId}）${sw.message ? `：${sw.message}` : ''}`);
    let res;
    let prod;
    let bonus;
    let trucks;
    ...
    let fields;
    try {
      res = parse2003(await mustRaw(c, 2003, Buffer.alloc(0)));
      prod = parse2026(await mustRaw(c, 2026, Buffer.alloc(0)));
      bonus = parse2018(await mustRaw(c, 2018, Buffer.alloc(0)));
      trucks = (parse19009(await mustRaw(c, 19009, Buffer.alloc(0)))
        .find((a) => a.id === 3) || {}).count || 0;
      militaryBuildings = parseBuildings49(await mustRaw(c, 17001, Buffer.alloc(0)), 1);
      fields = parseBuildings49(await mustRaw(c, 17002, Buffer.alloc(0)), 4);
    } catch (e) {
      // 单城读取失败不阻断全域：该城 D=0、无容量数据，调度与汇总按缺城处理，进度照常推进
      warn(`✗ 城池 ${city.cityId} 读取失败，已跳过：${e.message}`);
      continue;
    }
    ...
    // 目标兵种规格：逐厂读 3006（单价全域一致，但单件耗时受城内加成影响，必须按城取）。
    // 无厂或该厂不支持此兵种 → 该城产不出，D 与底仓线均为 0（不参与补料配给）。
    // 同一趟读回各厂训练队列深度：队列上限 = 厂等级（实测 L9=9 / L10=10，见 formula.trainQueueCap）
    const plantSpecs = [];
    const plantQueue = [];
    for (const plant of plants) {
      let info;
      try {
        info = parse3006(await mustRaw(c, 3006, p.u64(plant.bid)));
      } catch (e) {
        // 单厂读取失败：不计入产能统计，队列按满处理（份额转给可用厂，且不向该厂下单）
        warn(`✗ 城池 ${city.cityId} 厂规格读取失败，本厂不计入产能：${e.message}`);
        plantQueue.push({ bid: plant.bid, level: plant.level, depth: trainQueueCap(plant.level), cap: trainQueueCap(plant.level), free: 0 });
        continue;
      }
      const t = info.trainables[armyId];
      if (t) plantSpecs.push(t);
      const depth = (info.training || []).length;
      const cap = trainQueueCap(plant.level);
      plantQueue.push({ bid: plant.bid, level: plant.level, depth, cap, free: Math.max(0, cap - depth) });
    }
    // 单价兜底用数据表（与 3006 实测一致）；耗时缺失时该城 D=0，不用估算值冒充
    const tableArm = findArmy(armyId);
    const unitCost = plantSpecs.length
      ? { food: plantSpecs[0].food, steel: plantSpecs[0].steel, mineral: plantSpecs[0].mineral, oil: plantSpecs[0].oil }
      : (tableArm ? { ...tableArm.cost } : { food: 0, steel: 0, mineral: 0, oil: 0 });
    // 满负荷产能（件/h）：逐厂累加 3600/单件秒；无厂 → 0
    const Cin = armyCin(plantSpecs.map((t) => t.time));
    const scoutSec = plantSpecs.length ? plantSpecs[0].time / 1000 : 0;
    ...
    // D 与 floor 均由兵种决定（公式见 lib/formula.js 训练分区）
    const D = armyDemand(unitCost, Cin);
    const floor = armyFloor(D, floorHours);
    ...
    const s = {
      ...city,
      stock, cap, prod06: prod, res, P, cost: unitCost, armyId, scoutSec, Cin, D, floor, fieldCnt, special,
      plants,
      plantQueue,   // 各厂训练队列深度与余量（上限 = 厂等级）
