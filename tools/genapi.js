...
const SEMANTIC = [
  ...
  ['chat_time', '聊天时间戳'],
  ['chat_type', '消息类型'],
  ['attachment_flag', '附件标志（255 无附件 / 0 待领 / 1 已领）'],
  ['create_time', '创建时间戳'],
  ['start_time', '开始时间戳'],
  ['end_time', '结束时间戳'],
  ['status', '结果状态'],
...
const DOMAIN_HEAD = {
  '00-choice': [
    ...
    '| 平台 | 选服地址 | 协议形态 |',
    '|---|---|---|',
    '| iOS | `W2_SSO_CHOICE_HOST`（本地 .env 配置） | 裸 TCP，明文 WIST 帧 |',
    '| Android | 独立部署（WebSocket 端口） | WebSocket（裸 TCP 不响应） |',
    '',
    '服务端校验 platform/channel 与所选服务器匹配：平台不匹配时返回 `status=-1` 并附错误文案；连错平台地址则静默丢弃。客户端地址来自渠道分发配置（loginData 的 `choice_hosts`）。',
    '',
  ],
};
