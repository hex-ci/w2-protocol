...
test('HeaderBar 展示兵种与底仓线/油耗保留参数（参数可配后必须可见）', () => {
  const { lastFrame } = renderAt(html`<${HeaderBar} connected=${true} dry=${false} hubs=${''} army=${'侦察机'} floorHours=${24} fuelHours=${6} />`);
  const f = lastFrame();
  assert.match(f, /兵种: 侦察机/);
  assert.match(f, /底仓线 24h/);
  assert.match(f, /油耗保留 6h/);
});
