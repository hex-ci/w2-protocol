...
test('CostPanel 正常与阻塞两态', () => {
  const src = city();
  const dst = city({ cityId: '2', x: 100, y: 0 });
  const ok = render(html`<${CostPanel} src=${src} dst=${dst} loads=${{ food: 1200 }} fuelHours=${6} />`);
  assert.match(ok.lastFrame(), /✓ 约束全部通过/);
  assert.match(ok.lastFrame(), /能效比/);

  const empty = render(html`<${CostPanel} src=${src} dst=${dst} loads=${{}} fuelHours=${6} />`);
  assert.match(empty.lastFrame(), /✗ 未装载任何资源/);

  const busy = city({ slotsUsed: 10, slotsCap: 10 });
  const blocked = render(html`<${CostPanel} src=${busy} dst=${dst} loads=${{ food: 1200 }} fuelHours=${6} />`);
  assert.match(blocked.lastFrame(), /✗ 出征位已满/);
});

test('HeaderBar 显示连接态与 DRY 标记', () => {
  const a = render(html`<${HeaderBar} connected=${true} dry=${true} hubs=${'钢=城A'} />`);
  assert.match(a.lastFrame(), /已连接/);
  assert.match(a.lastFrame(), /DRY 模拟/);
  assert.match(a.lastFrame(), /中心仓: 钢=城A/);
  const b = render(html`<${HeaderBar} connected=${false} dry=${false} hubs=${''} />`);
  assert.match(b.lastFrame(), /未连接/);
});

test('TableLegend 覆盖全部着色语义', () => {
  const { lastFrame } = render(html`<${TableLegend} />`);
  const frame = lastFrame();
...
test('城池行渲染列对齐（宽度感知）', () => {
  ...
  const { lastFrame } = render(html`<${RowLine} cells=${cells} widths=${widths} inverse=${false} />`);
...
test('shipPlan 与面板显示一致（卡车数/油耗）', async () => {
  ...
  const { lastFrame } = render(html`<${CostPanel} src=${src} dst=${dst} loads=${loads} fuelHours=${6} />`);
  await tick();
  const frame = lastFrame();
